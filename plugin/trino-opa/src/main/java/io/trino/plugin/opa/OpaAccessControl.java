        ...
        permits OpaBatchAccessControl
{
    ...
    private final boolean opaRequestIncludePrincipal;
    ...
    public OpaAccessControl(LifeCycleManager lifeCycleManager, OpaHighLevelClient opaHighLevelClient, OpaConfig config, OpaPluginContext pluginContext)
    {
        ...
        this.opaRequestIncludePrincipal = config.getRequestIncludePrincipal();
    }
    ...
    {
        opaHighLevelClient.queryAndEnforce(buildQueryContext(identity), "ViewQueryOwnedBy", AccessDeniedException::denyViewQuery, OpaQueryInputResource.builder().user(new TrinoUser(queryOwner, opaRequestIncludePrincipal)).build());
    }
    ...
    public Collection<Identity> filterViewQueryOwnedBy(Identity identity, Collection<Identity> queryOwners)
    {
        return opaHighLevelClient.parallelFilterFromOpa(
                ...
                queryOwner -> buildQueryInputForSimpleResource(
                        ...
                        OpaQueryInputResource.builder().user(new TrinoUser(queryOwner, opaRequestIncludePrincipal)).build()));
    }

    @Override
    public void checkCanKillQueryOwnedBy(Identity identity, Identity queryOwner)
    {
        opaHighLevelClient.queryAndEnforce(buildQueryContext(identity), "KillQueryOwnedBy", AccessDeniedException::denyKillQuery, OpaQueryInputResource.builder().user(new TrinoUser(queryOwner, opaRequestIncludePrincipal)).build());
    }
    ...
    {
        return new OpaQueryContext(TrinoIdentity.fromTrinoIdentity(trinoIdentity,opaRequestIncludePrincipal), pluginContext);
    }

    OpaQueryContext buildQueryContext(SystemSecurityContext securityContext)
    {
        return new OpaQueryContext(TrinoIdentity.fromTrinoIdentity(securityContext.getIdentity(),opaRequestIncludePrincipal), pluginContext);
    }
}
